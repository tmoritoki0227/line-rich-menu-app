...
**機能は自由に変えてもらって構いません。家計簿でなくてもOKです**
...
| サービス | 役割 |
|---|---|
| **CloudFront** | フロントエンドを世界中に高速配信するCDN |
| **S3** | フロントエンドの静的ファイル（HTML/CSS/JS）を置く場所 |
| **API Gateway** | フロントエンドからのリクエストを受け付けるAPIの入口 |
| **Lambda** | ビジネスロジックを実行するサーバーレス関数 |
| **DynamoDB** | 支出データを保存するNoSQLデータベース |

### LINE

| サービス | 役割 |
|---|---|
| **LINE Developers** | チャネルを作成・管理する開発者向けコンソール |
| **LINEログインチャネル** | LIFFアプリを登録する。ユーザーのIDや名前を取得できる |
| **Messaging APIチャネル** | WebhookのURLを設定する。LINEへのメッセージ送信に使う |
| **LINE公式アカウント** | リッチメニューを設定する。ユーザーが触る入口 |
