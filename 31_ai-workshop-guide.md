...
開発効率を重視する方は、必ず最新モデル（Claude 3.5 Sonnet 等）が使える有料版を導入してください。無料版のAIには、**「全ての要件を読み込ませても、長文になると記憶が欠落し、構成図にある CloudFront や S3 の定義などを勝手に省略・無視する」**という致命的な限界があります。月20ドルの投資で、要件を隅々まで正確に読み取り、さらにターミナルを自律操作してデプロイまで完結させる「エージェント機能」が手に入ります。初心者が陥る「AIの指示通りにやったのに動かない」という挫折を物理的に回避するための必須投資です。
