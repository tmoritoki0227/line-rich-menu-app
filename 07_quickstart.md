...
### 4-2. スタック名のカスタマイズ（同一アカウントで複数ユーザーが使う場合）

`samconfig.toml` の `stack_name` はデプロイされる CloudFormation スタックの名前で、S3 バケット名もここから派生する（`{stack_name}-frontend`）。デフォルトは `line-rich-menu-app-stack` だが、**同一 AWS アカウントを複数人で使う場合は名前が衝突する**ため、自分の名前やチーム名を入れて変更する。

```toml
# samconfig.toml（変更箇所のみ）
stack_name = "line-rich-menu-app-自分の名前"   # 例: line-rich-menu-app-yamada
s3_prefix  = "line-rich-menu-app-自分の名前"   # stack_name と同じ値にする
```

変更後は以下のリソース名になる（例: `yamada`）：
- CloudFormation スタック: `line-rich-menu-app-yamada`
- S3 バケット: `line-rich-menu-app-yamada-frontend`

> 個人利用や専用アカウントの場合はデフォルトのままで構わない。

---

> **4-4 の `describe-stacks` コマンド内のスタック名** も変更した場合は合わせて書き換えること。
...
| `CloudFrontDistributionId` | GitHub Secrets の `CLOUDFRONT_DISTRIBUTION_ID` に設定 |
...
| `CLOUDFRONT_DISTRIBUTION_ID` | ステップ4-4 の `CloudFrontDistributionId` の値 |
