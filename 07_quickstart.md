...
| 4 | `.env` の作成と SAM デプロイ（AWS リソース一括作成） | 15分 |
...
2つのファイルを作成したら以下のコマンドを実行する（Mac/Linux・PowerShell 共通）。
...
| 内容 | 記事 |
|---|---|
| AWS IAM Identity Center の設定 | 記事① |
| LINE チャンネル・LIFF の詳細手順 | 記事② |
| LINE Webhook・LIFF エンドポイント・リッチメニュー設定 | 記事④ |
| GitHub Actions 用 IAM ロール作成の詳細 | 記事⑥「ステップ1」 |
| SAM テンプレートの詳細解説 | 記事⑥「ステップ2」 |
