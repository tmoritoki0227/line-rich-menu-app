...
Resources:
  ...
  FrontendBucket:
    Type: AWS::S3::Bucket
    Properties:
      BucketName: !Sub "${AWS::StackName}-frontend"  # スタック名ベースにすることで同一アカウント内でも一意になる
...
> **`Parameters` と `NoEcho: true` について**: `Parameters` はテンプレートに「この値はデプロイ時に外から渡す」と宣言するだけで、値そのものは書かない。値は `sam deploy --parameter-overrides` やGitHub Secretsから渡される。`NoEcho: true` を付けると CloudFormation のコンソールやログに値が `****` でマスクされ、トークンなど秘匿情報の漏洩を防ぐ。
